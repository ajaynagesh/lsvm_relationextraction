...
SVECTOR* find_cutting_plane(EXAMPLE *ex, SVECTOR **fycache, double *margin, long m, STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm, char* tmpdir, char *trainfile, char * regions_file, double Fweight) {
  ...
  time(&mv_start);
  find_most_violated_constraint_marginrescaling_all(ybar_all, hbar_all, sm, sparm, m, tmpdir, trainfile, regions_file);
  time(&mv_end);
...
double cutting_plane_algorithm(double *w, long m, int MAX_ITER, double C, double epsilon, SVECTOR **fycache, EXAMPLE *ex, STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm, char *tmpdir, char * trainfile, char * regions_file, double Fweight) {
  long i,j;
  double xi;
  double *alpha;
  ...
  new_constraint = find_cutting_plane(ex, fycache, &margin, m, sm, sparm, tmpdir, trainfile, regions_file, Fweight);
...
int main(int argc, char* argv[]) {
	...
	printf("Runs with F1 loss in the loss-augmented objective..  no dual decomposition!..");
...
//  exit(0);
  ...
  while ((outer_iter<2)||((!stop_crit)&&(outer_iter<MAX_OUTER_ITER))) {
    ...
    time(&cp_start);
    primal_obj = cutting_plane_algorithm(w, m, MAX_ITER, C, cooling_eps, fycache, ex, &sm, &sparm, learn_parm.tmpdir, trainfile, learn_parm.regions_file, learn_parm.Fweight);
    time(&cp_end);
