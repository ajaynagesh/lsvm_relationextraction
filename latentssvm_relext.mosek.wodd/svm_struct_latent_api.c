...
void find_most_violated_constraint_marginrescaling_all(LABEL *ybar_all, LATENT_VAR *hbar_all, STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm, int numEgs, char *tmpdir, char *trainfile, char * regions_file){

	// 1. Write input to a file
	char *params_filename = (char*) malloc(100);
	strcpy(params_filename, tmpdir);
	strcat(params_filename, "max_violator_all");

	write_to_file_params_t(sm->w, sparm->max_feature_key, sparm->total_number_rels, params_filename);
